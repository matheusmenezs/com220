...
class LimiteConsultaDisciplinas():
    def __init__(self):
        self.answer = simpledialog.askstring('Consulta Disciplinas', 'Digite o código: ')
...
class CtrlDisciplina():       
    ...
    def consultaDisciplinas(self):
        consultaIns = LimiteConsultaDisciplinas()
        self.msg = 'Disciplina: '
        for disc in self.listaDisciplinas:
            if consultaIns.answer == str(disc.getCodigo()):
                self.msg += disc.getCodigo() + '--' + disc.getNome() + '\n'
                consultaIns.mostraJanela('Consulta Disciplinas', self.msg)
                break
        else:
