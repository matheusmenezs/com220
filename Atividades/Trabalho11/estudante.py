...
class LimiteConsultaEstudantes():
    def __init__(self):
        self.answer = simpledialog.askstring("Consulta Estudantes", "Digite a matrícula: ")
...
class CtrlEstudante():       
    ...
    def consultaEstudantes(self):
        consultaIns = LimiteConsultaEstudantes()
        for estudante in self.listaEstudantes:
            self.msg = 'Estudante: '
            if consultaIns.answer == str(estudante.getNroMatric()):
                self.msg += estudante.getNroMatric() + ' -- ' + estudante.getNome() 
                consultaIns.mostraJanela('Consulta Estudantes', self.msg)
                break
        else:
            self.msg = 'Matrícula não cadastrada'
            consultaIns.mostraJanela('Consulta Estudantes', self.msg)
