...
class LimiteConsultaTurma():
    def __init__(self):
        self.answer = simpledialog.askstring('Consulta Turmas', 'Digite o código da disciplina: ')
...
class CtrlTurma():       
    ...
    def consultaTurma(self):
        consultaIns = LimiteConsultaTurma()
        
        self.msg = 'Turmas: \n'
        for disc in self.listaTurmas:
            if consultaIns.answer == str(disc.getDisciplina().getCodigo()):
                self.msg += disc.getCodigo()+ '--' + disc.getDisciplina().getCodigo()+ '--' + disc.getDisciplina().getNome() + '\n'

        if self.msg != 'Turmas: \n':
            consultaIns.mostraJanela('Consulta Turma', self.msg)
        else: 
            self.msg = 'Não há turmas com este código de disciplina'
            consultaIns.mostraJanela('Consulta Turma', self.msg)
